...
def check_user(user_id):
    check = sql.execute('SELECT * FROM users WHERE user_id=?;', (user_id,))
    if check.fetchone():
        return True
    else:
        return False
...
def make_order(user_id):
    pr_name = sql.execute('SELECT user_pr_name FROM cart WHERE user_id = ?;', (user_id,)).fetchone()
    user_pr_count = sql.execute('SELECT user_pr_count FROM cart WHERE user_id = ?', (user_id,)).fetchone()
    ...
    address = sql.execute('SELECT location FROM users WHERE user_id = ?;', (user_id,)).fetchone()
